    ...
    
{
    ...
    //uint32_t num_search_pnt=100;
    printf("num_pp_pairs=%d h_org_poly_idx_vec.size()=%zu h_pnt_len_vec.size()=%zu\n",
        num_pp_pairs,h_org_poly_idx_vec.size(),h_pnt_len_vec.size());
if(1)
{
...
if(1)
{
    int print_pp_pairs=(h_pnt_len_vec.size()<1000)?h_pnt_len_vec.size():1000;
    ...
    thrust::copy(pnt_ptr,pnt_ptr+print_pp_pairs,std::ostream_iterator<uint32_t>(std::cout, " "));std::cout<<std::endl;
    std::cout<<"d_pp_poly_idx"<<std::endl;
    thrust::copy(poly_ptr,poly_ptr+print_pp_pairs,std::ostream_iterator<uint32_t>(std::cout, " "));std::cout<<std::endl;    
}
...
std::unique_ptr<cudf::table> bbox_tbl_cpu(uint32_t num_poly,
    const double* h_x1, const double* h_x2,const double* h_y1,const double* h_y2)
    
{
    ...
    cudf::data_type{cudf::type_to_id<double>()}, num_poly,cudf::mask_state::UNALLOCATED, stream, mr);
    ...
    std::unique_ptr<cudf::table> bbox_tbl = 
    std::make_unique<cudf::table>(std::move(bbox_cols));
    return bbox_tbl;
}
