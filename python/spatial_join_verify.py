...
start = time.time()
np_pnt_x=points_df['x'].to_numpy()
np_pnt_y=points_df['y'].to_numpy()
...
start = time.time()
df=cudf.DataFrame({'map':key_to_point}, dtype=np.uint32)
#pnt_idx: offset in sorted point array
#match_idx: offset in orginal input point array
match_idx=df.take(pnt_idx)
end = time.time()
print('gather time', (end-start)*1000)

#np.setdiff1d is still too expensive to be practical;
#TODO: GPU-accelerated set difference
non_match_idx=np.setdiff1d(np.arange(total_points),match_idx.to_numpy())
